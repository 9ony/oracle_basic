...
//dim_cm 중 인덱스가 1번째인 값이 10보다 큰 상품들을 조회해서 출력하세요.
...
//upsert 옵션은 조건에 해당하는 문서가 있으면 수정 , 없으면 insert한다
...
db.inventory.updateMany({item:'postcard'},{$set:{"dim_cm.$[d]":12}},{arrayFilters:[{d:15.0}]})
...
db.board.find()
db.article.find()
db.article.deleteOne({writer:'chang'})
use boardDB
//아무글 1
db.article.insert({
    bid:db.board.findOne({name:"자유게시판"})._id,
    title:'자유게시판 실습1',
    content:'안녕하세요!',
    writer:'hong'
    })
//아무글 2
freeboard_res=db.board.findOne({name:'자유게시판'})
freeboard_id=freeboard_res._id
...
db.article.insertOne({
    bid:freeboard_id,
    title:'자유게시판 실습3',
    content:'안녕하세요 Min이에요',
    writer:'min'
})
db.article.updateMany({title:"자유게시판 실습3"},{$push:{
    comment:[
    {aid:db.article.findOne({title:"자유게시판 실습3"})._id},
    {comment:"반가워요 min님"},
    {writer:"choi"}
        ]}}
)
